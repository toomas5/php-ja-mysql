...
<body>
    <div class="container">
        <h3>Hinda kohta > <?php echo $row['nimi'];?></h3><br>
        ...
        if ($result_hinnangud->num_rows > 0) {
            while($row = $result_hinnangud->fetch_assoc()) {
                echo '<h5 class="mb-1">' . htmlspecialchars($row["nimi"]) . ' <span class="badge bg-warning">' . htmlspecialchars($row["hinnang"]) . '/10</span>';
                echo '<a href="/php/KT/admin/del_hinnang.php?kommentar=' . urlencode($row["kommentar"]) . '&koht=' . urlencode($valitudkoht) . '" style="text-decoration: none;" class="text-danger"> X</a></h5>';
